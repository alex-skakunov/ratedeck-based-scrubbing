...
class mysql extends db 
{
  ...
  public function __construct($host, $login, $password, $dbname)
  {
    parent::__construct($host, $login, $password, $dbname);
  }
  ...
  function connect()
  {
    if(extension_loaded('pdo_mysql')) {
        ...
        $this->link = new PDO($dsn, $this->login, $this->password, $options);
...
//    if($debug)
    ...
    if(extension_loaded('pdo_mysql')) {
        try {
            $this->fResult = $this->link->prepare($sql);
            $this->fResult->execute();
        } catch (PDOException $e) {
            $this->fError = $e->getMessage();
            $this->fErrorCode = $e->getCode();
        }
    }
    else {
...
//    echo $sql."<br/>";
  ...
  {
    if(extension_loaded('pdo_mysql')) {
        return $this->fResult->fetchColumn();
    }
    else {
